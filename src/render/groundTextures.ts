...
import { TextureLoader, MirroredRepeatWrapping, SRGBColorSpace, LinearMipmapLinearFilter, LinearFilter } from 'three';
...
export async function loadGroundTextures(base = 'assets/textures/'): Promise<GroundTextures> {
  ...
  await Promise.all(TEX_KEYS.filter(k => index[k]).map(k => new Promise<void>(resolve => {
    loader.load(base + index[k], t => { t.wrapS = t.wrapT = MirroredRepeatWrapping; t.colorSpace = SRGBColorSpace; t.minFilter = LinearMipmapLinearFilter; t.magFilter = LinearFilter; t.anisotropy = 4; out[k] = t; resolve(); }, undefined, () => { console.warn('بافت زمین بارگذاری نشد:', k); resolve(); });
  })));
  return out;
}
