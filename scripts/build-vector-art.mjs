...
// اصلاحیه‌ی ۴: ظاهر بالغ‌تر — خط دور نازک‌تر، رنگ‌ها کم‌اشباع‌تر و کمی پرکنتراست‌تر (آیکون‌ها و آرتیفکت‌ها دست‌نخورده)
const mature = svg => svg.replace(/stroke-width="([0-9.]+)"/g, (_, w) => `stroke-width="${(+w * 0.62).toFixed(2)}"`);
const png = (svgText, file, soft = true) => (soft ? sharp(Buffer.from(mature(svgText))).modulate({ saturation: 0.84, brightness: 0.97 }).linear(1.05, -3) : sharp(Buffer.from(svgText))).png().toFile(file);
const pngBuf = svgText => sharp(Buffer.from(mature(svgText))).modulate({ saturation: 0.84, brightness: 0.97 }).linear(1.05, -3).png().toBuffer();
...
for (const t of UNIT_TYPES) {
  ...
  const frames = await Promise.all([0, 1, 2, 3].map(i => pngBuf(unit(t, i))));
...
const doSet = async (group, gen, dir) => {
  for (const [name, fn] of Object.entries(gen)) {
    ...
    await png(s, `${root}/${file}`, group !== 'icons' && group !== 'artifacts');
